...
if __name__=="__main__":
    outputfile='csv_output/habmap_timedelta.csv'
    create_clean_csv(outputfile,'habName',True)
    [habnames,habID,habURL,erdURL]=get_assets('json_files/habmap_stations.json')
    for i in np.arange(0,len(habnames)):
        timedelta_str=get_asset_delta(habID[i],habURL[i])
        gsheet_status=get_gspread_status(hab_file='json_files/habmap_stations.json',hab_name=habnames[i])
