...
if __name__=="__main__":
    outputfile='csv_output/ifcb_timedelta.csv'
    create_clean_csv(outputfile,'ifcbName',True)
    [ifcbnames,ifcbID,ifcbURL]=get_assets('json_files/ifcb_names.json')
    for i in np.arange(0,len(ifcbnames)):
        timedelta_str=get_asset_delta(ifcbID[i],ifcbURL[i])
        gsheet_status=get_gspread_status(ifcb_file='json_files/ifcb_names.json',ifcb_name=ifcbnames[i])
